...
const ShopByCategory = () => {
    ...
    const [mobileView, setMobileView] = useState(false); //for mobile view
    ...
    return (
        <div className="category__page container">
            ...
            { mobileView<850 && <button
             className="btn show__sidebar__btn" 
             style={{color:sidebarOpen ? 'red' : '#3399ff'}}
             onClick={showLeftSidebar}>
             {sidebarOpen ? <RiCloseFill /> : <FaBars /> }</button>}
